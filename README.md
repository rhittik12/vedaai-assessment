...
```bash
npm --workspace apps/frontend run dev   # http://localhost:3000
...
| `OPENAI_API_KEY` | Key for generation |
...
1. Teacher uploads a source file via the UI
2. Backend saves it and queues a background job
3. Worker picks up the job, encodes the file, and sends it as a multimodal input to the model with a grounding prompt
4. Generated paper gets saved and linked back to the assignment
5. Teacher sees the result, can download or regenerate
